...
#    define KAN_HIGHLIGHT_SIZEOF_POSSIBLE(ARGUMENT) _Static_assert (sizeof (ARGUMENT) != 0u, "Highlight possible.");

/// \brief Macro that tells IDE that autocomplete should show enum names (argument should not have enum prefix).
#    define KAN_HIGHLIGHT_ENUM_NAME(ARGUMENT) _Static_assert (sizeof (enum ARGUMENT) != 0u, "Highlight possible.");

/// \brief Macro that tells IDE that autocomplete should show struct names (argument should not have struct prefix).
#    define KAN_HIGHLIGHT_STRUCT_NAME(ARGUMENT) _Static_assert (sizeof (struct ARGUMENT) != 0u, "Highlight possible.");

/// \brief Macro that tells IDE that autocomplete should show struct field name.
#    define KAN_HIGHLIGHT_STRUCT_FIELD(STRUCT, FIELD)                                                                  \
        _Static_assert ((sizeof ((struct STRUCT *) NULL)->FIELD) != 0u, "Highlight possible.");
#else
