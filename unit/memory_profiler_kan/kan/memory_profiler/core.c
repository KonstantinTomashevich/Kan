                                                              ...
                                                              const char *name)
{
    ...
    queue_new_allocation_group_event_unguarded (group);
    return group;
}
    ...
    kan_allocation_group_event_iterator_t event_iterator)
{
    ...
    const struct memory_event_node_t *node = (struct memory_event_node_t *) kan_event_queue_iterator_get (
        &event_queue, KAN_HANDLE_TRANSIT (kan_event_queue_iterator_t, event_iterator));
...
kan_allocation_group_event_iterator_t event_iterator_advance_unguarded (
    kan_allocation_group_event_iterator_t event_iterator)
{
    kan_event_queue_iterator_t iterator = KAN_HANDLE_TRANSIT (kan_event_queue_iterator_t, event_iterator);
    iterator = kan_event_queue_iterator_advance (iterator);
    cleanup_event_queue ();
    return KAN_HANDLE_TRANSIT (kan_allocation_group_event_iterator_t, iterator);
}

void event_iterator_destroy_unguarded (kan_allocation_group_event_iterator_t event_iterator)
{
    KAN_ASSERT (event_queue_initialized)
    kan_event_queue_iterator_destroy (&event_queue, KAN_HANDLE_TRANSIT (kan_event_queue_iterator_t, event_iterator));
    cleanup_event_queue ();
}
