...
void kan_resource_font_category_shutdown (struct kan_resource_font_category_t *instance)
{
    ...
    if (instance->precache_utf8)
    {
        kan_instance_size_t length = strlen (instance->precache_utf8);
        kan_free_general (instance->used_for_languages.allocation_group, instance->precache_utf8, length + 1u);
    }
}
