...
THREADING_API kan_thread_local_storage_t kan_thread_local_storage_create (void)
{
    ...
    if (tls == 0u)
    {
        ...
        return KAN_TYPED_ID_32_SET_INVALID (kan_thread_local_storage_t);
    }

    KAN_ASSERT (KAN_TYPED_ID_32_IS_VALID (KAN_TYPED_ID_32_SET (kan_thread_local_storage_t, tls)))
    return KAN_TYPED_ID_32_SET (kan_thread_local_storage_t, tls);
}
...
{
    if (SDL_SetTLS ((SDL_TLSID) KAN_TYPED_ID_32_GET (storage), value, destructor) != 0)
    {
...
{
    return SDL_GetTLS ((SDL_TLSID) KAN_TYPED_ID_32_GET (storage));
}
