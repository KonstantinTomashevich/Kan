...
UNIVERSE_RENDER_FOUNDATION_API KAN_UM_MUTATOR_GROUP_META (render_foundation_material_management,
                                                          KAN_RENDER_FOUNDATION_MATERIAL_MANAGEMENT_MUTATOR_GROUP);
    ...
    enum kan_render_pipeline_compilation_priority_t priority)
{
    ...
    KAN_UMI_VALUE_READ_REQUIRED (loaded, kan_render_material_loaded_t, name, &material_name)
...
static void create_new_usage_state_if_needed (struct render_foundation_material_management_planning_state_t *state,
                                              ...
                                              kan_interned_string_t material_name)
{
    KAN_UMI_VALUE_UPDATE_OPTIONAL (referencer_state, render_foundation_material_state_t, name, &material_name)
    if (referencer_state)
    {
...
{
    KAN_UMI_VALUE_WRITE_OPTIONAL (material, render_foundation_material_state_t, name, &material_name)
    if (!material)
    {
        return;
    }
    ...
    if (state->preload_materials && should_delete)
    {
        ...
        if (!should_delete)
        {
            ...
            update_loaded_compilation_priority (state, material_name, KAN_RENDER_PIPELINE_COMPILATION_PRIORITY_CACHE);
        }
    }
    ...
    KAN_UMI_VALUE_DELETE_OPTIONAL (loaded, kan_render_material_loaded_t, name, &material->name)
    if (loaded)
    {
    ...
    KAN_UMI_VALUE_WRITE_OPTIONAL (family_state, render_foundation_pipeline_family_state_t, name,
...
UNIVERSE_RENDER_FOUNDATION_API KAN_UM_MUTATOR_EXECUTE (render_foundation_material_management_planning)
{
...
UNIVERSE_RENDER_FOUNDATION_API
KAN_UM_MUTATOR_DEPLOY (render_foundation_material_management_execution)
{
    const struct kan_render_material_configuration_t *configuration = kan_universe_world_query_configuration (
        world, kan_string_intern (KAN_RENDER_FOUNDATION_MATERIAL_MANAGEMENT_CONFIGURATION));
                             ...
                             struct render_foundation_pipeline_family_state_t *family)
{
    ...
    KAN_UMI_VALUE_READ_REQUIRED (
        family_container, KAN_RESOURCE_PROVIDER_MAKE_CONTAINER_TYPE (kan_resource_material_pipeline_family_compiled_t),
        container_id, &family_request->provided_container_id)

    const struct kan_resource_material_pipeline_family_compiled_t *family_loaded =
        KAN_RESOURCE_PROVIDER_CONTAINER_GET (kan_resource_material_pipeline_family_compiled_t, family_container);

    if (family_loaded->vertex_attribute_sources.size > 0u || family_loaded->has_instanced_attribute_source)
    {
        attribute_sources = attribute_sources_static;
        attributes_sources_count = family_loaded->vertex_attribute_sources.size;

        if (family_loaded->has_instanced_attribute_source)
        {
        ...
        for (kan_loop_size_t index = 0u; index < family_loaded->vertex_attribute_sources.size; ++index)
        {
            struct kan_rpl_meta_attribute_source_t *source =
                &((struct kan_rpl_meta_attribute_source_t *) family_loaded->vertex_attribute_sources.data)[index];
        ...
        if (family_loaded->has_instanced_attribute_source)
        {
            attributes_count += family_loaded->instanced_attribute_source.attributes.size;
            attribute_sources[family_loaded->vertex_attribute_sources.size] =
                (struct kan_render_attribute_source_description_t) {
                    .binding = family_loaded->instanced_attribute_source.binding,
                    .stride = family_loaded->instanced_attribute_source.block_size,
    ...
    if (attributes_count > 0u)
    {
        ...
        for (kan_loop_size_t source_index = 0u; source_index < family_loaded->vertex_attribute_sources.size;
             ++source_index)
        {
            struct kan_rpl_meta_attribute_source_t *source = &(
                (struct kan_rpl_meta_attribute_source_t *) family_loaded->vertex_attribute_sources.data)[source_index];
            add_attributes_from_source (source, attributes, &attribute_output_index);
        }

        if (family_loaded->has_instanced_attribute_source)
        {
            add_attributes_from_source (&family_loaded->instanced_attribute_source, attributes,
                                        &attribute_output_index);
        }
    }

    push_constant_size = family_loaded->push_constant_size;
    if (family_loaded->set_material.buffers.size > 0u || family_loaded->set_material.samplers.size > 0u ||
        family_loaded->set_material.images.size > 0u)
    {
        char name_buffer[KAN_UNIVERSE_RENDER_FOUNDATION_NAME_BUFFER_LENGTH];
        snprintf (name_buffer, KAN_UNIVERSE_RENDER_FOUNDATION_NAME_BUFFER_LENGTH, "%s[set_material]", family->name);

        family->set_material = kan_render_construct_parameter_set_layout_from_meta (
            kan_render_backend_system_get_render_context (state->render_backend_system), &family_loaded->set_material,
    ...
    if (family_loaded->set_object.buffers.size > 0u || family_loaded->set_object.samplers.size > 0u ||
        family_loaded->set_object.images.size > 0u)
    {
        char name_buffer[KAN_UNIVERSE_RENDER_FOUNDATION_NAME_BUFFER_LENGTH];
        snprintf (name_buffer, KAN_UNIVERSE_RENDER_FOUNDATION_NAME_BUFFER_LENGTH, "%s[set_object]", family->name);

        family->set_object = kan_render_construct_parameter_set_layout_from_meta (
            kan_render_backend_system_get_render_context (state->render_backend_system), &family_loaded->set_object,
            name_buffer, state->description_allocation_group);
    ...
    if (family_loaded->set_shared.buffers.size > 0u || family_loaded->set_shared.samplers.size > 0u ||
        family_loaded->set_shared.images.size > 0u)
    {
        char name_buffer[KAN_UNIVERSE_RENDER_FOUNDATION_NAME_BUFFER_LENGTH];
        snprintf (name_buffer, KAN_UNIVERSE_RENDER_FOUNDATION_NAME_BUFFER_LENGTH, "%s[set_shared]", family->name);

        family->set_shared = kan_render_construct_parameter_set_layout_from_meta (
            kan_render_backend_system_get_render_context (state->render_backend_system), &family_loaded->set_shared,
            name_buffer, state->description_allocation_group);
    ...
    {
        KAN_UMI_VALUE_READ_REQUIRED (request, kan_resource_request_t, request_id, &pipeline_state->request_id)
        KAN_ASSERT (KAN_TYPED_ID_32_IS_VALID (request->provided_container_id))

        KAN_UMI_VALUE_READ_REQUIRED (
            container, KAN_RESOURCE_PROVIDER_MAKE_CONTAINER_TYPE (kan_resource_material_pipeline_compiled_t),
        ...
        if (kan_render_get_supported_code_format_flags () & (kan_memory_size_t) (1u << loaded->code_format))
        {
            ...
            {
                KAN_LOG (render_foundation_material, KAN_LOG_ERROR, "Cannot build code module for pipeline \"%s\".",
        ...
        KAN_UML_VALUE_UPDATE (pipeline_pass_variant, render_foundation_pipeline_pass_variant_state_t, pipeline_name,
                              &pipeline_state->pipeline_name)
        {
            ...
            KAN_UMI_VALUE_READ_OPTIONAL (pass, kan_render_graph_pass_t, name, &pipeline_pass_variant->pass_name)
            if (pass)
            {
...
#define ENTRY_POINTS_STATIC_COUNT 4u
                ...
                if (code_module_usage.entry_points_count > 0u)
                {
                    ...
                    for (kan_loop_size_t index = 0u; index < code_module_usage.entry_points_count; ++index)
                    {
                        ...
                        code_module_usage.entry_points[index] = (struct kan_render_pipeline_code_entry_point_t) {
                            .stage = stage,
                ...
                kan_render_pipeline_parameter_set_layout_t pass_parameter_set_layout = KAN_HANDLE_INITIALIZE_INVALID;
                if (pipeline_pass_variant->pass_variant_index < pass->variants.size)
                {
                ...
                struct kan_render_graphics_pipeline_description_t description = {
                    ...
                    .stencil_front =
                        {
                            .on_fail = convert_stencil_operation (loaded->pipeline_settings.stencil_front_on_fail),
                            .on_depth_fail =
                                convert_stencil_operation (loaded->pipeline_settings.stencil_front_on_depth_fail),
                            .on_pass = convert_stencil_operation (loaded->pipeline_settings.stencil_front_on_pass),
                            .compare = convert_compare_operation (loaded->pipeline_settings.stencil_front_compare),
                    ...
                    .stencil_back =
                        {
                            .on_fail = convert_stencil_operation (loaded->pipeline_settings.stencil_back_on_fail),
                            .on_depth_fail =
                                convert_stencil_operation (loaded->pipeline_settings.stencil_back_on_depth_fail),
                            .on_pass = convert_stencil_operation (loaded->pipeline_settings.stencil_back_on_pass),
                            .compare = convert_compare_operation (loaded->pipeline_settings.stencil_back_compare),
                ...
                {
                    kan_free_general (
                        state->description_allocation_group, color_outputs,
                        sizeof (struct kan_render_color_output_setup_description_t) * loaded->color_outputs.size);
                }
                ...
                {
                    kan_free_general (
                        state->description_allocation_group, code_module_usage.entry_points,
...
#undef AT_INDEX
    ...
    kan_dynamic_array_set_capacity (&loaded->vertex_attribute_sources, family_data->vertex_attribute_sources.size);
    ...
    for (kan_loop_size_t index = 0u; index < family_data->vertex_attribute_sources.size; ++index)
    {
        ...
        struct kan_rpl_meta_attribute_source_t *target = kan_dynamic_array_add_last (&loaded->vertex_attribute_sources);
        KAN_ASSERT (target)
                            ...
                            kan_time_size_t inspection_time_ns)
{
    ...
    KAN_UMI_VALUE_READ_OPTIONAL (family_request, kan_resource_request_t, request_id, &family->request_id)
    ...
    if (family_request)
    {
        ...
        else if (family_request->expecting_new_data ||
                 !KAN_TYPED_ID_32_IS_VALID (family_request->provided_container_id))
        {
            return;
        }
    }
    ...
    {
        KAN_UMI_VALUE_READ_OPTIONAL (pipeline_request, kan_resource_request_t, request_id, &pipeline->request_id)
        if (pipeline_request)
        {
            ...
            else if (pipeline_request->expecting_new_data ||
    ...
    {
        KAN_UMI_VALUE_UPDATE_OPTIONAL (loaded, kan_render_material_loaded_t, name, &material->name)
        if (loaded)
        {
...
static inline void on_pipeline_family_request_updated (
    ...
    kan_time_size_t inspection_time_ns)
{
    KAN_UMI_VALUE_UPDATE_OPTIONAL (family, render_foundation_pipeline_family_state_t, request_id, &request_id)
    if (family)
    {
                                                ...
                                                kan_time_size_t inspection_time_ns)
{
    kan_interned_string_t family_name;
    {
        KAN_UMI_VALUE_READ_OPTIONAL (pipeline_state, render_foundation_pipeline_state_t, request_id, &request_id)
        if (!pipeline_state)
        {
            return;
        }
    ...
    KAN_UMI_VALUE_UPDATE_REQUIRED (family, render_foundation_pipeline_family_state_t, name, &family_name)
    inspect_family (state, family, inspection_time_ns);
}
    ...
    struct render_foundation_material_pass_variant_state_t *material_pass)
{
    ...
    KAN_UMI_VALUE_UPDATE_OPTIONAL (pipeline, render_foundation_pipeline_state_t, pipeline_name,
                                   &material_pass->pipeline_name)
...
static inline void on_material_updated (struct render_foundation_material_management_execution_state_t *state,
                                        kan_resource_request_id_t request_id)
{
    KAN_UMI_VALUE_UPDATE_OPTIONAL (material, render_foundation_material_state_t, request_id, &request_id)
    if (!material)
    {
        return;
    }
    ...
    if (material_data->pipeline_family != material->pipeline_family_name)
    {
        ...
        if (material->pipeline_family_name != material->last_loaded_pipeline_family_name)
        {
            ...
            KAN_UMI_VALUE_UPDATE_OPTIONAL (family_to_add_reference, render_foundation_pipeline_family_state_t, name,
                                           &material_data->pipeline_family)

            if (family_to_add_reference)
            {
            ...
            else
            {
                KAN_UMO_INDEXED_INSERT (new_family, render_foundation_pipeline_family_state_t)
                {
                    ...
                    new_family->set_material = KAN_HANDLE_SET_INVALID (kan_render_pipeline_parameter_set_layout_t);
                    new_family->set_object = KAN_HANDLE_SET_INVALID (kan_render_pipeline_parameter_set_layout_t);
                    new_family->set_shared = KAN_HANDLE_SET_INVALID (kan_render_pipeline_parameter_set_layout_t);
    ...
    KAN_UML_VALUE_UPDATE (material_pass_to_unmark, render_foundation_material_pass_variant_state_t, material_name,
                          &material->name)
    {
        material_pass_to_unmark->usage_flags &= ~RENDER_FOUNDATION_MATERIAL_PASS_VARIANT_FOUND_IN_NEW_DATA;
    }

    // Mark pass variants that exists in new data or create new ones if they do no exist.
    for (kan_loop_size_t variant_index = 0u; variant_index < material_data->pass_variants.size; ++variant_index)
    {
        ...
        if (!found)
        {
            KAN_UMO_INDEXED_INSERT (new_material_pass, render_foundation_material_pass_variant_state_t)
            {
                ...
                new_material_pass->usage_flags = RENDER_FOUNDATION_MATERIAL_PASS_VARIANT_FOUND_IN_NEW_DATA;
                ...
                KAN_UMI_VALUE_READ_OPTIONAL (pass, kan_render_graph_pass_t, name, &variant->name)
    ...
    KAN_UML_VALUE_WRITE (material_pass_to_check, render_foundation_material_pass_variant_state_t, material_name,
                         &material->name)
    {
...
static inline void remove_pass_from_loaded_material (
    ...
    kan_interned_string_t pass_name)
{
    KAN_UMI_VALUE_UPDATE_OPTIONAL (loaded, kan_render_material_loaded_t, name, &material_name)
    if (!loaded)
    {
        return;
    }
...
UNIVERSE_RENDER_FOUNDATION_API KAN_UM_MUTATOR_EXECUTE (render_foundation_material_management_execution)
{
    ...
    KAN_UML_EVENT_FETCH (pass_updated_event, kan_render_graph_pass_updated_event_t)
    {
                              ...
                              &pass_updated_event->name)
        {
            ...
            {
                KAN_UMI_VALUE_READ_OPTIONAL (material, render_foundation_material_state_t, name,
